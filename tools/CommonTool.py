...
def show_question_message2(self, title, info):
        msg = QMessageBox(self)
        layout = msg.layout()
        #print(f"行数: {layout.rowCount()}, 列数: {layout.columnCount()}")  # 通常为 3 行 2 列
        #msg.setIcon(QMessageBox.Information)
        ...
        msg.button(QMessageBox.Save).setText("确定")  # 关键修改点
        msg.button(QMessageBox.Discard).setText("放弃")  # 关键修改点
        msg.button(QMessageBox.Cancel).setText("取消")  # 关键修改点
