...
from ui.FieldCalculatorUI import Ui_Dialog


class FieldCalculatorDialog(QDialog, Ui_Dialog):
    def __init__(self, parent=None):
        ...
        png = join(base_dir, 'settings/icon', 'FieldCalculator.png') 
        icon = QIcon()
    ...
    def GetValues(self):
        ...
        if selected_indexes:  # 确保有选中项
            ...
            for col in range(self.parent.tableModel.columnCount()):
                header_data = self.parent.tableModel.headerData(col, Qt.Horizontal)  # 获取列名
                if header_data == selected_value:
                    values = []
                    for row in range(self.parent.tableModel.rowCount()):
                        index = self.parent.tableModel.index(row, col)
                        value = self.parent.tableModel.data(index,Qt.DisplayRole)
                        #print(type(value))
                        values.append(value)
                    # 智能排序（数值优先，字符串次之）
                    def sort_key(x):
                        try:
                            return (0, float(x)) if x.replace('.','',1).isdigit() else (1, x.lower())
                        except:
                            return (2, x)  # 异常情况兜底

                    unique_sorted = sorted(list(set(values)), key=sort_key)              
                    self.valueListModel.setStringList(unique_sorted)
                    return
