...
from ui.ModelQueryUI import Ui_Dialog


class ModelQueryDialog(QDialog, Ui_Dialog):
    def __init__(self, parent=None):
        ...
        # 初始数据
        self.attributeListModel.setStringList(["名称", "类别", "分辨率","网络", "轮数", "损失(train)", "精度(train)", "损失(val)",  "查准(val)", "查全(val)", "IOU(val)"])
    ...
    def GetValues(self):
        ...
        if selected_indexes:  # 确保有选中项
            ...
            for col in range(self.parent.netModel.columnCount()):
                header_data = self.parent.netModel.headerData(col, Qt.Horizontal)  # 获取列名
                if header_data == selected_value:
                    values = []
                    for row in range(self.parent.netModel.rowCount()):
                        index = self.parent.netModel.index(row, col)
                        value = self.parent.netModel.data(index)
